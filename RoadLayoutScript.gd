...
onready var road_map_size = get_parent().road_map_size
var road_matrix = []
...
func generate_road_path():
	randomize()
	var rand_x = int(rand_range(0, road_map_size.x - 1))
	...
	while true:
		...
		if current_pos.y == road_map_size.y - 1:
...
func is_in_bounds(pos):
	if pos.x < 0 or pos.y < 0 or pos.x >= road_map_size.x or pos.y >= road_map_size.y:
		return false
	return true
